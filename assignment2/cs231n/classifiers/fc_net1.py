...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            ...
            self.params['W%d'%(layer+1)]=np.random.normal(0, weight_scale,layer_dim)
...
'''
        scores = None
        layers=[0]*(self.num_layers+1)
        cache=[0]*self.num_layers
        bn_cache=[0]*self.num_layers
        layers[0]=X
        for i in range(self.num_layers):
            wi=self.params['W%d'%(i+1)]
            bi=self.params['b%d'%(i+1)]
            if i==self.num_layers-1:
                layers[i+1],cache[i]=affine_forward(layers[i],wi,bi)
            else:
                if self.use_batchnorm:
                    gammai,betai=self.params['gamma%d' % (layer+1)],self.params['beta%d' % (layer+1)]
                    layers[i+1],cache[i]=affine_bn_relu_forward(layers[i],wi,bi,gammai,betai,self.bn_params[i])
                else:
                    layers[i+1],cache[i]=affine_relu_forward(layers[i],wi,bi)
        ...
        dout[self.num_layers]=dscores
        for i in range(self.num_layers,0,-1):
            if i==self.num_layers:
                dout[i-1],grads['W'+str(i)],grads['b'+str(i)]=affine_backward(dscores,cache[i-1])
            else:
                if self.use_batchnorm:
                    dx,dw,db,dgamma,dbeta=affine_bn_relu_backward(dout[i],cache[i-1])
                    dout[i-1], grads['W' + str(i)], grads['b' + str(i)] = dx, dw, db
                    grads['gamma'+str(i)], grads['beta' + str(i)] = dgamma, dbeta
                else:
                    dout[i-1],grads['W'+str(i)],grads['b'+str(i)]=affine_relu_backward(dout[i],cache[i-1])
